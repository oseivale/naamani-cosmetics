...
//     const email = formData.get("email"); // 🔹 so we can show it on confirmation page
...
//       // 🔁 Redirect to branded confirmation page instead of alert
//       router.push(
...
export default function CheckoutPage() {
  ...
  if (subtotal <= 75) {
    shipping = 20;
  } else if (subtotal >= 76 && subtotal <= 124) {
    shipping = 15;
  } else {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Redirect to confirmation page with info
      router.push(
        `/order-confirmed?orderNumber=${encodeURIComponent(
          data.orderNumber
        )}&total=${encodeURIComponent(
    ...
    <main className={styles.page}>
      <div className={styles.container}>
        ...
        <section className={styles.summary}>
          ...
          >
            <span className={styles.accordionTitle}>Order Summary</span>
            <span
              className={
                isSummaryOpen
